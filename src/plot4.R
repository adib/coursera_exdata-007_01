...
outputPlotFileName <- "../figure/plot4.png"
outputPlotWidth <- 480
...
plotVoltage <- function(dataset) {
  plot(x=dataset$Timestamp, y=dataset$Voltage,type="l",xlab="datetime",ylab="Voltage")
}


plotReactivePower <- function(dataset) {
  plot(x=dataset$Timestamp, y=dataset$Global_reactive_power,type="l",xlab="datetime",ylab="Global_reactive_power")
}

plotActivePower <- function(dataset) {
    plot(x=dataset$Timestamp, y=dataset$Global_active_power,type="l",xlab="",ylab="Global Active Power")    
}


plotSubMetering <- function(dataset) {
    ...
    legend(x="topright",legendText,lty=1,col=colors,box.lty=0)
}


plotAll <- function(dataset) {
    par(mfrow=c(2,2))
    plotActivePower(dataset)
    plotVoltage(dataset)
    plotSubMetering(dataset)
    plotReactivePower(dataset)    
}
